  ...
  return (
    <div className='flex flex-col justify-center items-center'>
      ...
      <div className='grid  grid-cols-6 mb-4'>
        <div className='col-span-2 flex justify-center items-center border-r border-dashed border-[#4F54A7] bg-white p-4 rounded-tr-lg rounded-br-lg rounded-tl-md rounded-bl-md font-bold uppercase text-xl'>{context.selectedCountry}</div>
        <div className='col-span-4 bg-[#9795f080] p-4 rounded-tr-lg rounded-br-lg rounded-tl-md rounded-bl-md font-bold flex justify-center flex-col relative border-l border-dashed border-[#4F54A7]'>
          <span className='absolute -top-2 right-2 text-6xl text-[#4F54A7]'>✈</span>
          {context.passengers.map((person, idx) =><p key={idx} >{`${person} - ${context.emails[idx]}`}</p> )}
        </div>
      </div>
